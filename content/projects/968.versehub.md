...
status: "In Progress"
draft: false
projectPage: "/projects/versehub"

liveSite: "https://versehub.app/"
...
heroStats:
  - icon: "mdi:database-search"
    value: "37K+"
    label: "Verses indexed (Weaviate)"
  - icon: "mdi:brain"
    value: "80%"
    label: "Requests served by local LLM"
  - icon: "mdi:timer-outline"
    value: "<200ms"
    label: "Avg. RAG retrieval latency"
  - icon: "mdi:cash-minus"
    value: "~70%"
...
37,000+ Bible and Quran verses indexed. RAG retrieval averaging under 200ms. 80% of inference requests served locally — reducing costs ~70% compared to routing everything through the OpenAI API. Semantic search surfaces relevant passages across both corpora without requiring the user to know a single verse reference. Zero request-path blocking on media uploads — FFmpeg transcoding fully decoupled via BullMQ. Five independently deployable system boundaries with no cross-domain coupling.

These are the outcomes. The architecture that produced them is a domain-driven SaaS platform with five bounded contexts: scripture content delivery, AI-assisted study, real-time community interaction, async media processing, and infrastructure services.

The core engineering challenge is not storing scripture — it is building a system that delivers contextually accurate AI responses, handles concurrent real-time sessions, and processes media asynchronously without blocking the main request path. Each problem requires a different architectural pattern; the platform composes them into a single deployment unit.
...
stats:
  ...
  - value: "80%"
    ...
    sub: "Llama 3.2 on Jetson GPU nodes"
  - value: "<200ms"
    label: "RAG Retrieval"
    sub: "Hybrid BM25 + vector search"
  - value: "~70%"
    label: "Cost Reduction"
    sub: "vs. full OpenAI API routing"
  - value: "3"
    label: "Async Queues"
...
VerseHub follows a modular, domain-driven structure. Each bounded context owns its own data access, service layer, and scaling surface. The Nitro server acts as the coordination layer without being a bottleneck — domain services communicate through typed interfaces, not shared tables.
...
## Observability & Operational Ownership

This system is pre-launch with no external users. The observability and deployment infrastructure was built first — not added after. That's an engineering decision: a system you can't measure isn't ready for users.

::HighlightGrid

::HighlightCard{icon="mdi:text-box-outline" title="Structured Logging — Loki"}
Loki aggregates structured logs from all containers with correlation IDs threaded through API, worker, and AI service calls. Every PostgreSQL query, Weaviate retrieval, BullMQ dispatch, and inference call is traceable back to the originating request. Queried directly from Grafana alongside metrics.
::
...
::HighlightCard{icon="logos:prometheus" title="Prometheus + Grafana + Alertmanager"}
Prometheus scrapes request latency, BullMQ queue depth, worker throughput, and per-model inference time. Grafana dashboards surface queue backlog, Redis memory pressure, and Jetson GPU utilization. Alertmanager routes on sustained queue stalls, high error rates, and inference latency spikes.
::

::HighlightCard{icon="mdi:source-branch" title="Automated CI/CD — Gitea Actions"}
Self-hosted Gitea Actions runners execute on every push: lint, type-check, integration tests, Docker image build, registry push, and rolling Kubernetes deploy. No manual deploy steps. A failed test blocks the deploy — the pipeline is the gatekeeper.
::
