...
const views = [
  { id: 'routing',  label: 'Cost-Aware Routing' },
  { id: 'breakeven', label: 'Break-Even Model' },
]

const stages = {
  routing: [
    { label: 'Request', icon: 'mdi:api', color: 'slate', lines: ['Classify complexity'] },
    { label: 'Local Model', icon: 'mdi:home-circle', color: 'emerald', lines: ['~Marginal cost only', 'Standard requests'] },
    { label: 'Hosted API', icon: 'mdi:cloud', color: 'sky', lines: ['Per-token billed', 'Hard/complex requests'] },
    { label: 'Response', icon: 'mdi:check-circle', color: 'amber', lines: ['Cost logged per route'] },
  ],
  breakeven: [
    { label: 'Hardware Cost', icon: 'mdi:expansion-card', color: 'purple', lines: ['One-time purchase'] },
    { label: 'Displaced Spend', icon: 'mdi:cash-minus', color: 'sky', lines: ['Monthly API/subscription'] },
    { label: 'Electricity', icon: 'mdi:flash', color: 'amber', lines: ['Ongoing marginal cost'] },
    { label: 'Break-Even Point', icon: 'mdi:calendar-check', color: 'emerald', lines: ['Months to ROI'] },
  ],
}

const legends = {
  routing: [
    { color: '#34d399', label: 'Local inference' },
    { color: '#38bdf8', label: 'Hosted API' },
  ],
  breakeven: [
    { color: '#c084fc', label: 'Fixed cost' },
    { color: '#fbbf24', label: 'Ongoing cost' },
    { color: '#34d399', label: 'ROI outcome' },
  ],
}
