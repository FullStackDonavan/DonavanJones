  ...
  'request-flow': [
    { id: 'user',    type: 'arch', position: { x: 0,    y: 210 },
      data: { label: 'Client',      icon: 'mdi:monitor',          color: 'slate',
              lines: ['Browser / API consumer'] } },
    { id: 'ingress', type: 'arch', position: { x: 240,  y: 210 },
      data: { label: 'Ingress',     icon: 'logos:nginx',           color: 'sky',
              lines: ['TLS termination', 'Domain routing', 'Rate limiting'] } },
    { id: 'fastapi', type: 'arch', position: { x: 480,  y: 210 },
      data: { label: 'FastAPI',     icon: 'simple-icons:fastapi',  color: 'green',
              lines: ['REST endpoints', 'Validation', 'Background tasks'] } },
    { id: 'postgres',type: 'arch', position: { x: 760,  y: 20  },
      data: { label: 'PostgreSQL',  icon: 'logos:postgresql',      color: 'emerald',
              lines: ['Primary relational DB', 'SQLAlchemy ORM'] } },
    { id: 'redis',   type: 'arch', position: { x: 760,  y: 210 },
      data: { label: 'Redis',       icon: 'logos:redis',           color: 'amber',
              lines: ['Cache + sessions', 'Job queue backend'] } },
    { id: 'ai',      type: 'arch', position: { x: 760,  y: 390 },
      data: { label: 'AI Compute',  icon: 'mdi:memory',            color: 'violet',
              lines: ['RTX 3090 inference', 'Embedding generation'] } },
    { id: 'minio',   type: 'arch', position: { x: 1040, y: 210 },
      data: { label: 'MinIO',       icon: 'simple-icons:minio',    color: 'blue',
              lines: ['Object storage', 'Signed URL delivery'] } },
  ],

  'ai-pipeline': [
    { id: 'api',      type: 'arch', position: { x: 0,    y: 180 },
      data: { label: 'FastAPI',          icon: 'simple-icons:fastapi',  color: 'green',
              lines: ['Receives request', 'Enqueues job'] } },
    { id: 'queue',    type: 'arch', position: { x: 260,  y: 180 },
      data: { label: 'Redis Queue',      icon: 'logos:redis',           color: 'amber',
              lines: ['Job dispatch', 'ARQ / Celery backend'] } },
    { id: 'orch',     type: 'arch', position: { x: 520,  y: 180 },
      data: { label: 'AI Orchestrator',  icon: 'mdi:brain',             color: 'violet',
              lines: ['Prompt construction', 'Model routing', 'Context mgmt'] } },
    { id: 'llm',      type: 'arch', position: { x: 800,  y: 20  },
      data: { label: 'LLM — RTX 3090',  icon: 'mdi:memory',            color: 'purple',
              lines: ['Llama inference', 'Embedding models', 'RAG pipeline'] } },
    { id: 'weaviate', type: 'arch', position: { x: 800,  y: 180 },
      data: { label: 'Weaviate',         icon: 'mdi:vector-polyline',   color: 'sky',
              lines: ['Vector embeddings', 'Semantic search', 'RAG retrieval'] } },
    { id: 'edge',     type: 'arch', position: { x: 800,  y: 360 },
      data: { label: 'Jetson Orin Nano', icon: 'mdi:chip',              color: 'cyan',
              lines: ['Vision models', 'Edge inference', 'On-device processing'] } },
    { id: 'store',    type: 'arch', position: { x: 1080, y: 180 },
      data: { label: 'MinIO',            icon: 'simple-icons:minio',    color: 'blue',
              lines: ['Model artifacts', 'Datasets', 'Output storage'] } },
  ],

  'infrastructure': [
    { id: 'gitea',  type: 'arch', position: { x: 0,   y: 200 },
      data: { label: 'Gitea CI',      icon: 'simple-icons:gitea',        color: 'orange',
              lines: ['Source control', 'CI runner', 'Docker build + push'] } },
    { id: 'k3s',    type: 'arch', position: { x: 270, y: 200 },
      data: { label: 'k3s on RPi 5',  icon: 'logos:kubernetes',          color: 'sky',
              lines: ['ARM64 cluster', 'Pod scheduling', 'Service LB'] } },
    { id: 'app',    type: 'arch', position: { x: 400, y: 20  },
      data: { label: 'App Pods',       icon: 'mdi:application-brackets',  color: 'green',
              lines: ['FastAPI replicas', 'Worker pods', 'Rolling deploys'] } },
    { id: 'data',   type: 'arch', position: { x: 840, y: 200 },
      data: { label: 'Data Pods',      icon: 'mdi:database',              color: 'emerald',
              lines: ['Postgres', 'Redis', 'Weaviate'] } },
    { id: 'ai-pod', type: 'arch', position: { x: 400, y: 380 },
    ...
    { id: 'obs',    type: 'arch', position: { x: 840, y: 20  },
      data: { label: 'Observability',  icon: 'logos:prometheus',          color: 'amber',
              lines: ['Prometheus', 'Grafana', 'Alertmanager'] } },
    { id: 'net',    type: 'arch', position: { x: 840, y: 390 },
      data: { label: 'Networking',     icon: 'mdi:network',               color: 'slate',
              lines: ['Internal DNS', 'Firewall rules', 'Service mesh (planned)'] } },
  ],
}
