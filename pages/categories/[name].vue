...
useSeoMeta({
  ...
  description: () => `${totalArticlesCount.value ?? 0} articles in the ${categoryName.value} category on donavanjones.com — engineering breakdowns, architecture decisions, and real-world production lessons.`,
  ...
  ogDescription: () => `Browse ${totalArticlesCount.value ?? 0} ${categoryName.value} articles on donavanjones.com — engineering breakdowns, architecture decisions, and lessons from production systems.`,
  ogType: 'website',
    ...
    <div class="w-full">
      ...
      <div class="bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
        <div class="max-w-4xl mx-auto px-6 py-16 sm:py-24 text-center">
          ...
          <h1 class="text-4xl sm:text-5xl font-bold tracking-tight text-slate-900 dark:text-white capitalize">
            {{ categoryName }}
          </h1>
          <p class="mt-4 text-base text-slate-500 dark:text-slate-400">
            {{ totalArticlesCount ?? 0 }} article{{ (totalArticlesCount ?? 0) !== 1 ? 's' : '' }} in this category
          </p>
        </div>
      </div>
